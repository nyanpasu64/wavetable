...
# GAUSS = A(57, 967, 967, 57) / 2048
GAUSS = l(255)
LOLGAUSS = normalize_filter(GAUSS ** 1.5)
...
def circular_convolve(raw, filter):
    ...
    # while len(new) < l0 + len(filter):
    #     new = np.concatenate(new, raw)

    # l = len(raw)
    # hl = l//2
    if len(filter) > len(raw):
        raise ValueError('raw %s < filter %s' % (raw, filter))

    new = cat(raw, raw)
    del raw

    conv = np.convolve(new, filter, 'same')
    assert len(new) == len(conv)
...
def repeatedly_convolve(raw, filter, n):
    for i in range(n):
        raw = circular_convolve(raw, filter)
